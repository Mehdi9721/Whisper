...
// },[message.newMessage,message.messages])
    ...
    const handleNavigate = (userId) => {
      // Find the chat associated with the selected user
                  ...
                  <div className="flex justify-between items-center p-3">
                    ...
                    >
                      <img
                        ...
                        alt="profile"
                      />
                    <p>{auth.reqUser && auth.reqUser.name}</p>
  ...
  <div className="bg-white overflow-y-scroll h-[73vh] px-3">
    <div className="chats-container ">
      {querys &&
        Array.isArray(auth.searchUser) &&auth.searchUser?.map((item, index) => (
          <div
            key={index}
      ...
      {chat.chats?.length > 0 &&
        ...
        chat.chats?.map((item, index) => (
          <div key={index} onClick={() => handleCurrentChat(item)}>
            ...
            {!currentChat?.id && (
              <div className="w-[70%] flex flex-col items-center justify-center h-full">
                <div className="max-w-[70%] text-center">
                  ...
                  <h1 className="text-4xl text-gray-600">Online ChatApp</h1>
                  <p className="my-9">
            ...
            {/* Message Section */}
            {currentChat?.id && (
              <div className="w-[80%] relative  bg-blue-200">
    ...
    : (currentChat.users[0] && currentChat.users[1]) ? (
        auth.reqUser?.id !== currentChat.users[0]?.id
    ...
    messages?.map((item, i) => (
      <MessageCard
        key={i}
        isReqUserMessage={item.user && item.user.id !== auth.reqUser.id}
      ...
      />
                ...
                {/* Footer Section */}
                <div className="footer bg-[#f0f2f5] relative bottom-0 w-full py-2 text-xl">
                  <div className="flex justify-between items-center px-5 relative">
                    <BsEmojiSmile className="cursor-pointer" />
                    <ImAttachment />

                    <input
                      className="py-1 outline-none border-none bg-white pl-4 rounded-md w-[85%]"
                      type="text"
